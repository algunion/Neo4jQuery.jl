...
"""
Extract the Cypher string from a @macroexpand'd macro expression.
...
an expanded macro expression, preserving insertion order.
"""
...
# No live database is needed — all tests verify compile-time behavior
# using @macroexpand and direct function calls.
# ════════════════════════════════════════════════════════════════════════════
...
@testset "DSL" begin
    ...
    @testset "End-to-end: social graph DSL scenario" begin
        ...
        ex = @macroexpand @cypher conn begin
            a::SocialPerson >> f::FOLLOWS >> b::SocialPerson
            where(a.age > $min_age, !(f.muted))
            ret(a.name => :follower, b.name => :following, f.since => :year)
            order(f.since, :desc, a.name, :asc)
            take(50)
        end
